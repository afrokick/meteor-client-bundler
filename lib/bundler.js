...
function bundle(options) {
  var sourceDir = options.source && Path.resolve(cwd, options.source);
  var packsDir = options.packsDir && Path.resolve(cwd, options.packsDir);
  ...
  // fix package.json
  var packageJsonPath = Path.resolve(tempDir, "package.json");
  var packageJsonContent = JSON.parse(
    Fs.readFileSync(packageJsonPath).toString()
  );
  if (packageJsonContent && packageJsonContent.meteor) {
    delete packageJsonContent.meteor;
    Fs.writeFileSync(
      packageJsonPath,
    ...
    .forEach(function (pack) {
      ...
      if (config.escapeCommonJS) {
        if (config.externalNpmPackages) {
          for (const packageName of config.externalNpmPackages) {
            packContent = packContent
              .split(`require("${packageName}`)
              .join(`unescapedRequire("${packageName}`);
            packContent = packContent
              .split(`require('${packageName}`)
              .join(`unescapedRequire('${packageName}`);
          }
        ...
        if (config.externalNpmPackages) {
          for (const packageName of config.externalNpmPackages) {
            packContent = packContent
              .split(`unescapedRequire("${packageName}`)
